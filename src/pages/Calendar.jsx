...
const MOCK_RAIDS = [
    {
        ...
        loot: [
            { itemId: 28773, itemName: 'Gorehowl',        quality: 'epic', slot: 'Arma principal', winner: 'Arthas',  boss: 'Prince Malchezaar', icon: 'inv_axe_18' },
            { itemId: 28767, itemName: 'Malchazeen',      quality: 'epic', slot: 'Arma principal', winner: 'Illidan', boss: 'Prince Malchezaar', icon: 'inv_dagger_20' },
            { itemId: 28749, itemName: "King's Defender", quality: 'epic', slot: 'Arma principal', winner: 'Uther',   boss: 'Chess Event',       icon: 'inv_sword_63' },
        ]
    },
    { id: 2, title: "Gruul's Lair",    date: 'Jueves, 20:00 ST',   signups: [], loot: [] },
    { id: 3, title: 'Heroico / alters', date: 'Domingo, 19:30 ST', signups: [
...
function Calendar() {
    ...
    const handleCreateRaid = (e) => {
        ...
        setRaids([...raids, { id: Date.now(), title: newRaidTitle, date: newRaidDate, signups: [], loot: [] }]);
        setNewRaidTitle('');
        setNewRaidDate('');
    };
    ...
    const handleSignUp = (raidId) => {
        ...
        setRaids(raids.map(raid => {
            ...
            if (raid.signups.some(su => su.name === charName)) { alert("¡Ya estás apuntado a esta raid!"); return raid; }
            return { ...raid, signups: [...raid.signups, { name: charName, class: charClass, role: charRole }] };
        }));
    };

    const handleDeleteRaid  = (id) => setRaids(raids.filter(r => r.id !== id));
    const handleAddLoot     = (raidId, entry) => setRaids(raids.map(r => r.id !== raidId ? r : { ...r, loot: [...(r.loot ?? []), entry] }));
    const handleRemoveLoot  = (raidId, idx)   => setRaids(raids.map(r => r.id !== raidId ? r : { ...r, loot: r.loot.filter((_, i) => i !== idx) }));
                ...
                <div className="flex-1 min-w-[300px] max-w-[350px] max-[900px]:max-w-full">
                    {isAdmin ? (
                        ...
                        <div className="glass-panel p-8">
                            ...
                            <p className="text-[0.9rem] text-[#8b8b99] mb-6">Registra tus detalles antes de apuntarte a un evento.</p>
                                ...
                                {loot.map((entry, idx) => (
                                    <div key={idx} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 px-[0.75rem] py-[0.6rem] bg-[rgba(255,255,255,0.02)] rounded-[6px] border border-[rgba(255,255,255,0.03)] transition-all duration-150 hover:bg-[rgba(255,255,255,0.05)] hover:border-[rgba(255,255,255,0.08)]">
                                        <div className={`item-info-row ${entry.quality} flex items-center gap-[0.6rem] min-w-0`}>
                                            ...
                                            <span className="item-name font-medium text-[0.88rem] whitespace-nowrap overflow-hidden text-ellipsis">{entry.itemName ?? entry.item}</span>
                                        </div>
                                        ...
                                        {isAdmin && (
                                            <button
                                                onClick={() => handleRemoveLoot(raid.id, idx)}
